...
class BaseModel extends Model
{
    ...
    */
    public function listPageBy($where=null,$field='',$page=1,$page_size=1000,$order='id desc')
    {
        ...
        if($field)
        {
            $res = $m->where($where)->limit($offset,$page_size)->order($order)->field($field)->select(); 
        }
        else
        {
            $res = $m->where($where)->limit($offset,$page_size)->order($order)->select();
        }
    ...
    */
    ...
    {
        $m = new $this->class;
        return $m->where($where)->setInc("point",$field_val);
    }
    ...
    public function dec($where,$field_name,$field_val)
    {
        ...
        $m = new $this->class;
        return $m->where($where)->setDec($field_name,$field_val);
    }
    ...
    */
    ...
    {
        $m = new $this->class;
        return $this->where('id = '.$id)->field($field)->order($order)->find(); 
    }
