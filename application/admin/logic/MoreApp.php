...
class MoreApp extends BaseLogic
{
    /*******************************基础配置开始*******************************/
    
    public $flag_opt = array();
    public $id_alias = "app_gift_id";//主表与其他表关联的字段
    ...
    public function appClick($id){
        ...
        return model('more_app')->appClick($id);
    }

    public function viewListBy($ids){
        $sql = 'select * from s_more_app where id in('.$ids.') order by sort desc';
        return Db::query($sql);
    }
}